...
char* indexes(const char* path)
{
    ...
    if(p == NULL)
    {
        strcat(str1,"index.php\0");
        strcat(str2,"index.html\0");

        if(access(str1, F_OK ) != -1 )
        {
            res = str1;
            return res;
        } else if(access(str2, F_OK) != -1) {
            res = str2;
            return res;
        }
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    char temp[50];
    strcpy(temp,line);
    char full_request[4][20];
    int req = 0, http = 0;
    char *p = NULL, *p2 = NULL;

    for (int i = 0, j = 0, lenght = strlen(temp); i < lenght; i++)
    {
        if(temp[i] == ' ' || i == lenght - 2 || temp[i] == '?')
        {
            full_request[req][j] = '\0';
            req++;
            j = 0;
        } else {
        full_request[req][j] = temp[i];
        j++;
        }
    }

    strcpy(abs_path, full_request[1]);
    p = strchr(abs_path,'.');
    p2 = strchr(temp,'?');
    ...
    if (strcmp(full_request[0],"GET") != 0)
    {
        error(405);
    } else if(full_request[0][1] == '/')
    {
        error(501);
    } else if(full_request[0][1] == '"')
    {
        error(400);
    } else if(strcmp(full_request[http], "HTTP/1.1") != 0)
    {
        error(505);
    } else {
        return true;
...
void transfer(const char* path, const char* type)
{
    ...
    struct stat st;
    stat(path2, &st);
    length = st.st_size;    if (load(file, &content, &length) == false)
    {
        error(500);
        return;
    }
